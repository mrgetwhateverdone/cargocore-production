    ...
    try {
      ...
      if (Array.isArray(insights) && insights.length > 0) {
        return insights.slice(0, 3).map((insight: any) => ({
          ...
          content: safeFormatAIText(insight.content) || ''
...
function parseTextResponseToInsights(content: string, template: any): ReportInsight[] {
  ...
  return sections.slice(0, 3).map((section, index) => ({
    title: safeFormatAIText(`${template.name} Insight ${index + 1}`) || `${template.name} Insight ${index + 1}`,
    content: safeFormatAIText(section.trim()) || section.trim(),
